  ...
  static nodesMovieCard(state, props, toggleCommentsPanel, toggleVotePanel) {
    ...
    nodes.panelToggles = (
      <div className="btn-group pull-right">
        <a className="btn-primary btn" onClick={ toggleCommentsPanel}>{ state.showCommentsPanel ? 'Hide' : 'Comments' }</a>
        <a className="btn-primary btn" onClick={ toggleVotePanel}>{ state.showVotePanel ? 'Hide' : 'Vote' }</a>
